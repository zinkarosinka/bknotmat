...
class Player(sprite.Sprite):
    ...
    def player1Move(self):
        ev = key.get_pressed()
        if ev[K_1] and self.rect.y >= 0:
            self.rect.y -=  self.yspeed
        if ev[K_2] and self.rect.y <= 500-self.h:
            self.rect.y += self.yspeed

    def player2Move(self):
        ev = key.get_pressed()
        if ev[K_9] and self.rect.y <= 500-self.h:
            self.rect.y += self.yspeed
        if ev[K_0] and self.rect.y >= 0:
            self.rect.y -= self.yspeed
...
pl_group1 = sprite.Group()
pl_group2 = sprite.Group()
pl_group1.add(player1)
pl_group2.add(player2)
...
def collider():
    sprite_groupe = sprite.groupcollide(
        pl_group1, ball_group, False, False
    )
